...
> [!tip] Shared defaults
> Listen, masquerade, and QUIC windows live in **one** file: `cloud/common/stacks/hysteria/defaults.yaml`.
> No server `bandwidth` (Brutal off → default BBR). Each VPS only has `site.yaml` (ACME/TLS + password). Compose mounts both and `cat`s them at container start.
...
| **Compose** | `cloud/hosts/ali/hysteria/` |
...
| **Compose** | `cloud/hosts/digi/hysteria/` |
...
```
cloud/common/stacks/hysteria/defaults.yaml        # listen, masquerade, quic (no bandwidth / BBR)
cloud/common/stacks/hysteria/docker-compose.yml   # canonical compose (cat defaults+site at boot)
cloud/common/stacks/hysteria/up.sh                # --project-directory hosts/<host>/hysteria
cloud/common/stacks/hysteria/render.py            # preview merge on stdout
cloud/hosts/<host>/hysteria/site.yaml             # acme + auth only
cloud/hosts/<host>/hysteria/acme/                 # cert storage
```
...
# no bandwidth — Brutal off; default CC is BBR
...
mkdir -p /allah/blue/cloud/hosts/NEWHOST/hysteria/acme
...
```bash
mkdir -p acme
bash /allah/blue/cloud/common/stacks/hysteria/up.sh ali
...
```bash
python3 cloud/common/stacks/hysteria/render.py ali
```
...
```bash
cd /allah/blue/network/cloudflare
...
Omit `up`/`down` / `brutal-opts` so Hy2 uses BBR (current policy). Re-add matching hints only if you intentionally re-enable Brutal:
...
```bash
bash cloud/common/stacks/hysteria/up.sh ali
docker logs -f hysteria2
docker restart hysteria2
docker ps | grep hysteria
ss -ulnp | grep 443
# Confirm no Brutal bandwidth block (BBR):
python3 cloud/common/stacks/hysteria/render.py ali | grep -E '^(bandwidth|listen|quic):' || true
```
...
- Preview: `python3 cloud/common/stacks/hysteria/render.py ali`
