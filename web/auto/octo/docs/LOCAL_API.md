...
- **Base URL**: `http://localhost:59999` — port is configured in `start_octo.sh`
- **API prefix**: `/api/v2/`
...
**Full example:**
```bash
curl -s -X POST "http://localhost:59999/api/v2/profiles" \
  -H "Content-Type: application/json" \
  -d '{
...
**Example:**
```bash
curl -s -X PATCH "http://localhost:59999/api/v2/profiles/61fe4cf012f446deb14443ca0d9d9ebb" \
  -H "Content-Type: application/json" \
  -d '{
...
**Example:**
```bash
curl -s -X POST "http://localhost:59999/api/v2/profiles/61fe4cf012f446deb14443ca0d9d9ebb/clone" \
  -H "Content-Type: application/json" \
  -d '{"amount": 3}'
```
...
**Example:**
```bash
curl -s -X POST "http://localhost:59999/api/v2/profiles/61fe4cf012f446deb14443ca0d9d9ebb/start" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
**Full example:**
```bash
curl -s -X POST "http://localhost:59999/api/profiles/one_time/start" \
  -H "Content-Type: application/json" \
  -d '{
    "profile_data": {
        "fingerprint": {
...
- **App entry**: `http://localhost:59999/new/app/start` (or `/new/app/start?v=...&client_uuid=...`)
...
```bash
curl -s http://localhost:59999/api/v2/client/themes && echo " OK" || echo " Down"
```
