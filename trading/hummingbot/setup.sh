...
FT_HUMMINGBOT_DIR="/allah/blue/trading/hummingbot"
...
# Write connector config from secrets/cred.json
echo "Writing connector configuration from cred.json..."
python3 - <<'PY'
import sys
from pathlib import Path
sys.path.insert(0, str(Path("/allah/blue/workstation/scripts")))
from cred_loader import get_binance
bn = get_binance()
out = Path("/allah/hummingbot/conf/connectors/binance_perpetual.yml")
out.write_text(
    "connector: binance_perpetual\n"
    f"binance_perpetual_api_key: {bn['api_key']}\n"
    f"binance_perpetual_api_secret: {bn['api_secret']}\n",
    encoding="utf-8",
)
print("wrote", out)
PY
