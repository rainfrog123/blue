#!/usr/bin/env python3
"""Build xray-reality config.json from common defaults + hosts/<host>/site.json."""
...
def merge(host: str) -> dict:
    site_file = CLOUD / "hosts" / host / "xray-reality" / "site.json"
...
def main() -> int:
    ...
    p.add_argument("host", help="hosts/<host> short name (digi|ali|azure|ali-jp|...)")
    ...
    if args.stdout:
        ...
        out = CLOUD / "hosts" / args.host / "xray-reality" / "config.json"
        write_json(out, data)
