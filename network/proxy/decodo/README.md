...
```bash
cd network/proxy/decodo
pip install -r requirements.txt
```

## Credentials

Via shared `workstation/scripts/cred_loader.py` → `get_proxy_decodo()` / `get_ipqs()`
(`blue/secrets/cred.json`). Env overrides: `DECODO_USERNAME`, `DECODO_PASSWORD`,
