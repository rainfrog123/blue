...
function Modify-CursorJSFiles {
    ...
    # 部署外置 Hook 文件（供 Loader Stub 加载）
    # 修改: 使用本地 Hook 文件，不再从远程下载
    $hookTargetPath = "$env:USERPROFILE\.cursor_hook.js"
    
    # 本地 Hook 文件路径候选列表（按优先级排序）
    $hookSourceCandidates = @()
    
    # 1. 脚本同目录下的 cursor_hook.js
    if (-not [string]::IsNullOrWhiteSpace($PSScriptRoot)) {
        $hookSourceCandidates += (Join-Path $PSScriptRoot "cursor_hook.js")
        $hookSourceCandidates += (Join-Path $PSScriptRoot "..\hook\cursor_hook.js")
    } elseif ($MyInvocation.MyCommand.Path) {
        $scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
        if (-not [string]::IsNullOrWhiteSpace($scriptDir)) {
            $hookSourceCandidates += (Join-Path $scriptDir "cursor_hook.js")
    ...
    if (-not [string]::IsNullOrWhiteSpace($cwdPath)) {
        $hookSourceCandidates += (Join-Path $cwdPath "cursor_hook.js")
    ...
    if ($hookSourcePath) {
        try {
            ...
            Write-Host "$GREEN✅ [Hook]$NC 本地 Hook 已部署: $hookTargetPath"
            Write-Host "$BLUEℹ️  [Hook]$NC 源文件: $hookSourcePath"
        } catch {
            Write-Host "$RED❌ [Hook]$NC 本地 Hook 复制失败: $($_.Exception.Message)"
        }
    } else {
        Write-Host "$RED❌ [Hook]$NC 未找到本地 Hook 文件 (cursor_hook.js)"
        Write-Host "$YELLOW💡 [提示]$NC 请确保 cursor_hook.js 与脚本在同一目录"
        Write-Host "$YELLOW💡 [搜索路径]$NC"
        foreach ($candidate in $hookSourceCandidates) {
