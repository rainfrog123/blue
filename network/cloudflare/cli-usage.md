...
Credentials are automatically loaded from `/allah/blue/secrets/cred.json` (no setup needed).

Fallback order:
1. `CLOUDFLARE_API_TOKEN` env var
2. `CLOUDFLARE_EMAIL` + `CLOUDFLARE_API_KEY` env vars
3. `/allah/blue/secrets/cred.json` → `cloudflare.email` + `cloudflare.global_api_key`
4. `/allah/blue/secrets/cred.json` → `cloudflare_alt.api_token`
