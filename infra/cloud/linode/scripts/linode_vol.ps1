...
$linodeToken = $cred.linode.tokens[3]  # Use token index 3
...
# Shut down the Linode (use a different token)
$HeadersShutdown = @{
    "Authorization" = "Bearer $($cred.linode.tokens[4])"  # Use token index 4
    "Content-type" = "application/json"
}
