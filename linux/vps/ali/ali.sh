...
# Create sysctl.conf if it doesn't exist and load BBR module
...
# 🛡️ Ubuntu 24.04: Override any cloud-init or drop-in configs
sudo mkdir -p /etc/ssh/sshd_config.d/
echo -e "PermitRootLogin yes\nPasswordAuthentication no" | sudo tee /etc/ssh/sshd_config.d/99-custom.conf > /dev/null

# 🚀 Restart SSH service to apply changes (Ubuntu 24.04 compatible)
