...
build_auth_string() {
    local session_num=$1
    local auth_string="${DECODO_USERNAME}-sessionduration-${session_duration}"
...
echo "Proxy host: $PROXY_HOST_HTTPS"
echo "Port range: $PROXY_PORT_HTTPS_MIN-$PROXY_PORT_HTTPS_MAX (random)"
...
test_session() {
    ...
    local proxy_port=$(get_random_https_port)
    local proxy_url="https://${auth_string}@${PROXY_HOST_HTTPS}:${proxy_port}"
...
# Process results in numerical order
for i in $(seq 1 $num_sessions); do
    result_file="${TEMP_DIR}/session_${i}.result"
    if [ -f "$result_file" ]; then
        ...
        if [[ "$first_field" == "FAIL" ]]; then
            ...
            if ! $duplicate; then
                ...
                proxy_links["$ip"]="https://$clean_cmd@$PROXY_HOST_HTTPS:$proxy_port"
