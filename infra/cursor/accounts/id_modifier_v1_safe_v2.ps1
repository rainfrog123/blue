...
function Modify-CursorJSFiles {
    ...
    # 目标JS文件列表（Windows路径）
    # SAFE V2: extensionHostProcess.js用APPEND，其他文件用PREPEND
    $jsFiles = @(
    ...
    # 特殊处理的文件（使用APPEND方式）
    $appendOnlyFile = "extensionHostProcess.js"
    
    Write-Host "$GREEN✅ [安全模式V2]$NC extensionHostProcess.js用APPEND，其他文件用PREPEND"
    ...
    foreach ($file in $jsFiles) {
        ...
        try {
            ...
            # 检查是否已经修改过
            if ($content -match "// Cursor ID Safe V2 Injection" -or $content -match "CursorDeviceIdPatch_IIFE" -or $content -match "// Cursor ID 修改工具注入") {
                Write-Host "$GREEN✅ [跳过]$NC 文件已经被修改过: $fileName"
                $modifiedCount++
                continue
            }
            ...
            # 根据文件类型选择注入方式
            if ($fileName -eq $appendOnlyFile) {
                # extensionHostProcess.js 使用 APPEND 方式（IIFE）- 保护扩展功能
                $injectCode = @"

// Cursor ID Safe V2 Injection - $(Get-Date) - IIFE APPEND方式（保护扩展功能）
;(function CursorDeviceIdPatch_IIFE() {
    'use strict';
    try {
        ...
        console.log('[Safe V2] Device ID patched - APPEND mode');
    } catch (e) { console.error('[Safe V2] Patch error:', e.message); }
})();
"@
                Write-Host "$BLUE🔧 [Safe V2]$NC $fileName 使用APPEND方式（保护Jupyter等扩展）..."
                $content = $content + $injectCode
                Write-Host "$GREEN✅ [成功]$NC IIFE注入成功（追加到文件末尾）"
            }
            else {
                # main.js 和 cliProcessMain.js 使用 PREPEND 方式（原始方法）
                $injectCode = @"
...
console.log('Cursor设备标识符已成功劫持 - PREPEND mode');

"@
                Write-Host "$BLUE🔧 [原始方法]$NC $fileName 使用PREPEND方式..."
                $content = $injectCode + $content
                Write-Host "$GREEN✅ [成功]$NC ES模块注入成功（插入到文件开头）"
            }
...
function Modify-MachineCodeConfig {
    ...
    if (-not (Test-Path $configPath)) {
        ...
        # 自动尝试启动Cursor生成配置文件
        Write-Host "$BLUE🚀 [自动尝试]$NC 正在自动尝试启动Cursor生成配置文件..."
...
"@
Write-Host "$BLUE================================$NC"
Write-Host "$GREEN🚀   Cursor 防掉试用Pro删除工具 (安全版V2)   $NC"
...
# 🎯 自动选择模式 1（仅修改机器码）
Write-Host ""
Write-Host "$GREEN🎯 [自动模式]$NC 自动执行：仅修改机器码"
Write-Host ""
Write-Host "$BLUE  1️⃣  仅修改机器码$NC (已自动选择)"
...
Write-Host "$GREEN✅ [自动选择]$NC 已自动选择：仅修改机器码"
Write-Host ""
...
# 🤔 自动确认执行
Write-Host "$GREEN🤔 [自动确认]$NC 自动确认执行..."
Write-Host "$GREEN✅ [确认]$NC 已自动确认继续执行"
Write-Host ""
