...
each tile "invokes mcp" — i.e. the 'Planning next moves' indicator clears after
the `mcp` follow-up is submitted.

Phases:
  1. arm     — for each tile: Ctrl+D split -> Auto -> prompt -> Opus -> type `mcp`
               -> submit. Records the tile index and its submit timestamp (t0).
  2. monitor — round-robin HOLD Enter on each pending tile (bursts) and watch its
               planning state. A tile is "done" once planning has appeared and
               then stayed cleared (debounced). Global safety cap (default 6 min).
  3. log     — print a summary table and write a JSON log next to this script.
               Tiles that never cleared within the cap are logged as TIMEOUT
               (did NOT invoke mcp).
...
Run from anywhere:
    python "C:\\Users\\jar71\\blue\\linux\\cursor\\batch_run.py" --tiles 6
"""
import argparse
import json
import os
...
def arm_tiles(ws, n, prompt_text, type_text):
    ...
    for i in range(n):
        ...
        tt = type_text or wf.opus_prompt()
        idx = wf.run_phase(ws, prompt, idx)
