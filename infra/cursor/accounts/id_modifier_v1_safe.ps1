...
function Modify-CursorJSFiles {
    ...
    # 目标JS文件列表（Windows路径）
    # SAFE VERSION: extensionHostProcess.js is EXCLUDED to prevent breaking extensions (Jupyter, etc.)
    $jsFiles = @(
        "$cursorAppPath\resources\app\out\main.js",
        "$cursorAppPath\resources\app\out\vs\code\node\cliProcessMain.js"
    )
    
    Write-Host "$YELLOW⚠️  [安全模式]$NC 已跳过 extensionHostProcess.js 以保护扩展功能"
...
"@
Write-Host "$BLUE================================$NC"
Write-Host "$GREEN🚀   Cursor 防掉试用Pro删除工具 (安全版)   $NC"
