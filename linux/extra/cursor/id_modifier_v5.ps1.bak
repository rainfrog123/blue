            ...
            if ((Split-Path $file -Leaf) -eq "main.js") {
                ...
                try {
                    ...
                    foreach ($hm in $hashMatches) {
                        ...
                        if ($funcStart -lt 0) {
                            if ($candidateNo -le 3) { $diagLines += "Candidate#${candidateNo}: async function not found" }
                            continue
                        }
                        ...
                        if ($endBrace -lt 0) {
                            if ($candidateNo -le 3) { $diagLines += "Candidate#${candidateNo}: brace matching failed" }
                            continue
                        }

                        $funcText = $windowText.Substring($funcStart, $endBrace - $funcStart + 1)
                        if ($funcText.Length -gt 8000) {
                            if ($candidateNo -le 3) { $diagLines += "Candidate#${candidateNo}: function too long len=$($funcText.Length), skipped" }
                            continue
                        }

                        $sig = [regex]::Match($funcText, '^async function (\w+)\((\w+)\)')
                        if (-not $sig.Success) {
                            if ($candidateNo -le 3) { $diagLines += "Candidate#${candidateNo}: function signature not parsed" }
                            continue
                        }
