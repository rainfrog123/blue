...
# Generate IDs using new function
$MAC_MACHINE_ID = New-StandardMachineId
$UUID = [System.Guid]::NewGuid().ToString()
# Convert auth0|user_ to byte array hex
$prefixBytes = [System.Text.Encoding]::UTF8.GetBytes("auth0|user_")
$prefixHex = -join ($prefixBytes | ForEach-Object { '{0:x2}' -f $_ })
# Generate 32 bytes (64 hex chars) of random data for machineId random part
...
try {
    ...
    if (-not (Test-Path $STORAGE_FILE)) {
        Write-Host "$RED[ERROR]$NC Config file not found: $STORAGE_FILE"
    ...
    try {
        ...
        Write-Host "$GREEN[INFO]$NC Config file updated successfully"
    } catch {
    ...
    # Execute Update-MachineGuid directly, no prompting
    Update-MachineGuid
    # Display results
    Write-Host ""
    ...
    # Ask if user wants to disable auto-update
    Write-Host ""
    Write-Host "$YELLOW[PROMPT]$NC Do you want to disable Cursor auto-update?"
    ...
    if ($choice -eq "1") {
        ...
        try {
            ...
            if (Test-Path $updaterPath) {
                # If it's a file, blocking update is already created
    ...
    try {
        ...
        Write-Host "$GREEN[INFO]$NC Config written successfully using alternative method"
    } catch {
...
function Write-ConfigFile {
    ...
    try {
        ...
        # Standardize using LF line breaks
        ...
        Write-Host "$GREEN[INFO]$NC Config file written successfully (UTF8 without BOM)"
    }
    catch {
