...
Credentials come from ``workstation/scripts/cred_loader.get_proxy_decodo()``
(``blue/secrets/cred.json`` → ``proxy.decodo``). Env vars still override.
"""
...
def _ensure_cred_loader() -> None:
    scripts = Path(__file__).resolve().parents[4] / "workstation" / "scripts"
